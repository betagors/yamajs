 ...
 * ```
 */
export function isYamaError(error) {
 ...
 * ```
 */
export function normalizeError(error, defaultOptions = {}) {
    // Already a YamaError
    if (isYamaError(error)) {
        return error;
    }
 ...
 */
export function createError(message, code, options = {}) {
    return new YamaError(message, {
        ...options,
        code,
 ...
 */
export function withSuggestions(error, suggestions) {
    const yamaError = normalizeError(error);
    return new YamaError(yamaError.message, {
        code: yamaError.code,
        statusCode: yamaError.statusCode,
 ...
 */
export function getSafeErrorMessage(error, isProduction = process.env.NODE_ENV === 'production') {
