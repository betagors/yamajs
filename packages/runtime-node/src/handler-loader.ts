 ...
 * including import resolution for @yamajs/* and @gen/* packages.
 */
...
import type { HandlerFunction } from "@yamajs/kernel";

/**
 * Resolve @yamajs/* and @gen/* imports using package.json exports
 * 
...
export function resolveYamaImports(handlerContent: string, projectRoot: string, fromPath: string): string {
  try {
    ...
    // Replace @yamajs/* and @gen/* imports with resolved paths
    let resolvedContent = handlerContent;
    for (const [exportPath, exportValue] of Object.entries(exports)) {
      if (exportPath.startsWith("@yamajs/") || exportPath.startsWith("@gen/")) {
 ...
 * - Automatically resolves @yamajs/* and @gen/* imports
...
export async function loadHandlerByPath(handlerPath: string, configDir: string): Promise<HandlerFunction | null> {
  ...
  if (!existsSync(absoluteHandlerPath)) {
    console.warn(`âš ï¸  Handler file not found: ${absoluteHandlerPath}`);
    return null;
  }
  ...
  try {
    ...
    // Resolve @yamajs/* and @gen/* imports if package.json exists
    if (existsSync(packageJsonPath)) {
    ...
    if (typeof handlerFn === "function") {
      console.log(`âœ… Loaded handler: ${handlerPath}`);
      return handlerFn;
    } else {
      console.warn(`âš ï¸  Handler ${handlerPath} does not export a function (expected ${handlerName} or default)`);
      return null;
    }
  } catch (error) {
    console.error(`âŒ Failed to load handler ${handlerPath}:`, error);
    return null;
  }
}
