...
export function generateOpenAPI(config: YamaConfig): OpenAPISpec {
  ...
  for (const endpoint of allEndpoints) {
    ...
    try {
      ...
      console.error(
        `âŒ Error: Failed to convert endpoint "${endpoint.method} ${endpoint.path}" to OpenAPI operation.\n` +
