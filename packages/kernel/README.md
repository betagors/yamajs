...
The cache adapter interface provides a unified API for cache operations. Cache adapters are typically provided via plugins (e.g., `@yamajs/cache-redis`).
...
const plugin = await loadPlugin('@yamajs/db-postgres');
...
- Cache adapters are provided via plugins (e.g., `@yamajs/cache-redis`)
