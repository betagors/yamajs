...
export async function createDataSnapshot(
  ...
  try {
    ...
    // Add metadata (PGlite doesn't support COMMENT ON TABLE, so we'll skip it)
    // The snapshot table name already contains timestamp information
...
export async function listSnapshots(config: DatabaseConfig): Promise<Array<{
  ...
  try {
    ...
    for (const row of result as unknown as Array<{ table_name: string }>) {
      ...
      // Extract timestamp from table name if possible
      const timestampMatch = row.table_name.match(/_snapshot_(\d+)$/);
