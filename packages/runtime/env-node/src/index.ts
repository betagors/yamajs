...
// Dynamic import for http-fastify (peer dependency)
let _fastifyAdapter: typeof import("packages/http-fastify") | null = null;

async function getFastifyAdapter() {
  if (!_fastifyAdapter) {
    try {
      _fastifyAdapter = await import("packages/http-fastify");
    } catch (error) {
      throw new Error(
...
async function getOpenApiAdapter() {
  try {
    ...
    const openapiModule = await import("packages/codegen/openapi");
...
export async function startYamaNodeRuntime(
  ...
  if (yamlConfigPath) {
    try {
      ...
      await loadEnvFile(yamlConfigPath, environment);
