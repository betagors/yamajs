...
export async function generateMigration(
  ...
  const currentVersion = await getCurrentSchemaVersion(projectDir);
  const previousEntities = currentVersion
    ? await loadEntitySnapshot(projectDir, currentVersion.version)
    : null;
 ...
 * Check if entities have changed since last recorded version
 */
export async function hasEntityChanges(
  projectDir: string,
  currentEntities: YamaEntities
): Promise<boolean> {
  const currentVersion = await getCurrentSchemaVersion(projectDir);
