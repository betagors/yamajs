...
export async function hashPassword(
  ...
  return getRuntime().crypto.hashPassword(password, saltRounds);
}
...
export async function verifyPassword(
  ...
  return getRuntime().crypto.verifyPassword(password, hash);
}
...
export function generateSecureToken(length: number = 32): string {
  const bytes = getRuntime().crypto.randomBytes(length);
...
export function generateOTP(length: number = 6): string {
  // Simple implementation using randomBytes since randomInt isn't in runtime
  const bytes = getRuntime().crypto.randomBytes(4); // 32 bits is enough for usual OTPs
...
export function secureCompare(a: string, b: string): boolean {
  return getRuntime().crypto.timingSafeEqual(a, b);
}
