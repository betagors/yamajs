...
export async function getPluginPackageDir(packageName, projectDir) {
    try {
        ...
        throw new Error(`Could not resolve package directory for ${packageName}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Ensure plugin migration tables exist
 */
export async function ensurePluginMigrationTables(sql) {
    try {
        await sql.unsafe(PLUGIN_MIGRATIONS_TABLE_SQL);
        await sql.unsafe(PLUGIN_VERSIONS_TABLE_SQL);
    }
    catch (error) {
        throw new Error(`Failed to create plugin migration tables: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Get installed plugin version from database
 */
export async function getInstalledPluginVersion(pluginName, sql) {
    try {
 ...
 */
export async function getPendingPluginMigrations(plugin, manifest, installedVersion, currentVersion) {
    ...
    if (!semver.valid(currentVersion)) {
        console.warn(`Plugin ${plugin.name} has invalid version "${currentVersion}", skipping migrations`);
        return [];
    }
    ...
    const migrationVersions = Object.keys(migrations)
        .filter((v) => {
        if (!semver.valid(v)) {
            console.warn(`Plugin ${plugin.name} has invalid migration version "${v}", skipping`);
            return false;
        }
        if (!installedVersion) {
        ...
        if (!semver.valid(installedVersion)) {
            console.warn(`Plugin ${plugin.name} has invalid installed version "${installedVersion}", treating as first install`);
            return semver.lte(v, currentVersion);
        }
 ...
 */
async function loadMigrationScript(migration, pluginDir, direction) {
    const script = direction === "up" ? migration.up : migration.down;
    if (!script) {
        throw new Error(`Migration ${direction} script not found`);
    }
 ...
 */
export async function executePluginMigration(migration, sql, pluginDir) {
 ...
 */
export async function rollbackPluginMigration(pluginName, toVersion, sql, pluginDir, manifest) {
  ...
  `;
    ...
    for (const applied of migrationsToRollback) {
        ...
        if (!migrationDef) {
            throw new Error(`Migration definition not found for version ${version} in plugin ${pluginName}`);
        }
        if (!migrationDef.down) {
            throw new Error(`Rollback migration (down) not defined for version ${version} in plugin ${pluginName}`);
        }
 ...
 */
export async function updatePluginVersion(pluginName, version, sql) {
 ...
 */
export async function getPluginMigrationHistory(pluginName, sql) {
    try {
        const result = await sql `
