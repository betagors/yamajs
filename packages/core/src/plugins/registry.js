...
import { ensurePluginMigrationTables, getInstalledPluginVersion, getPendingPluginMigrations, executePluginMigration, updatePluginVersion, getPluginPackageDir, } from "./migrations.js";
import { PluginContextImpl } from "./context.js";
import { validateDependencies, } from "./dependencies.js";
import { trackPluginInit, recordPluginInitialized, recordPluginError, } from "./metrics.js";
/**
 * Plugin registry
 */
export class PluginRegistry {
     ...
     */
    setConfig(config, projectDir, logger) {
        this.config = config;
    ...
    createContext() {
        ...
        return new PluginContextImpl(this.config, this.projectDir, this.logger, this.plugins, this.pluginAPIs, this.middlewareRegistry);
    }
    /**
     ...
     */
    async loadPlugin(packageName, projectDir, pluginConfig = {}, skipDependencies = false) {
        ...
        if (!skipDependencies && manifest.dependencies?.plugins) {
            ...
            if (!validation.valid) {
                throw new Error(`Plugin ${packageName} has unsatisfied dependencies: ${validation.missing.join(", ")}`);
            }
        }
        // Import plugin
        ...
        try {
            ...
            console.warn(`Could not resolve package directory for ${packageName}, migrations may not work: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        if (manifest.security) {
            ...
            if (securityValidation.errors.length > 0) {
                throw new Error(`Security validation failed for plugin ${packageName}: ${securityValidation.errors.join(", ")}`);
            }
        }
        ...
        if (manifest.configSchema && pluginConfig) {
            ...
            if (!configValidation.valid) {
                throw new Error(`Invalid configuration for plugin ${packageName}: ${configValidation.errors?.join(", ")}`);
            }
        }
        ...
        if (manifest.migrations && Object.keys(manifest.migrations).length > 0) {
            try {
                ...
                if (dbConnection) {
                    ...
                    const installedVersion = await getInstalledPluginVersion(packageName, sql);
                    // Get plugin version
                    ...
                    if (pending.length > 0) {
                        ...
                        if (!packageDir) {
                            console.warn(`Cannot run migrations for ${packageName}: package directory not found`);
                        }
                        else {
                            ...
                            for (const migration of pending) {
                                try {
                                    ...
                                    console.log(`  ✅ Migrated ${packageName} from ${migration.fromVersion} to ${migration.toVersion}`);
                                    ...
                                    console.error(`  ❌ Migration failed for ${packageName} from ${migration.fromVersion} to ${migration.toVersion}: ${err.message}`);
                            ...
                            try {
                                ...
                                console.warn(`Failed to update version record for ${packageName}: ${error instanceof Error ? error.message : String(error)}`);
                            }
                        }
                    }
                }
                    ...
                    if (manifest.migrations) {
                        console.warn(`⚠️  Plugin ${packageName} has migrations but no database plugin is available. Migrations will be skipped.`);
                    }
                }
            }
            catch (error) {
                // Migration system error - log but don't fail plugin load
                console.warn(`⚠️  Failed to run migrations for ${packageName}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        return plugin;
    }
    ...
    getAllCLICommands() {
        ...
        for (const context of this.pluginContexts.values()) {
            ...
            allCommands.push(...context.getCLICommands());
        }
        return allCommands;
    }
    /**
    ...
    getAllMCPTools() {
        ...
        for (const context of this.pluginContexts.values()) {
            // All contexts are PluginContextImpl instances
            allTools.push(...context.getMCPTools());
        }
        return allTools;
    }
    /**
 ...
 */
export function setPluginRegistryConfig(config, projectDir, logger) {
 ...
 */
export async function loadPlugin(packageName, projectDir, pluginConfig = {}) {
 ...
 * Get plugin API
 */
export function getPluginAPI(packageName) {
 ...
 * Get all registered CLI commands from all plugins
 */
export function getAllCLICommands() {
