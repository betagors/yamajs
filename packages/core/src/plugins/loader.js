...
export async function loadPluginFromPackage(packageName, projectDir) {
    ...
    try {
        ...
        throw new PluginError(`Plugin package not found: ${packageName}`, {
            code: ErrorCodes.PLUGIN_NOT_FOUND,
            context: { packageName },
    ...
    if (!yamaMetadata) {
        throw new PluginError(`Plugin ${packageName} does not have "yama" metadata in package.json`, {
...
export async function importPlugin(manifest, packageName, projectDir) {
    ...
    try {
        ...
        throw new PluginError(`Could not resolve entry point for plugin ${packageName}`, {
    ...
    try {
        ...
        if (!plugin) {
            throw new PluginError(`Plugin ${packageName} does not export a plugin`, {
                code: ErrorCodes.PLUGIN_CONFIG_INVALID,
                context: { packageName },
                suggestions: [
        ...
        if (typeof plugin.init !== "function") {
            throw new PluginError(`Plugin ${packageName} does not implement init() method`, {
                code: ErrorCodes.PLUGIN_CONFIG_INVALID,
                context: { packageName },
                suggestions: [
        ...
        throw new PluginError(`Failed to import plugin ${packageName}`, {
            code: ErrorCodes.PLUGIN_INIT_FAILED,
            context: { packageName },
            cause: error instanceof Error ? error : undefined,
        });
