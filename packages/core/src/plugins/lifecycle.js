...
export class PluginLifecycleManager {
    ...
    register(plugin) {
        ...
        if (!validation.valid) {
            ...
            error.code = ErrorCodes.PLUGIN_CONFIG_INVALID;
            throw error;
        }
        // Check if already registered
        if (this.plugins.has(plugin.name)) {
            const error = new Error(`Plugin ${plugin.name} is already registered`);
            error.code = ErrorCodes.CONFLICT_EXISTS;
            throw error;
        }
        ...
        if (!versionResult.valid) {
            ...
            error.code = ErrorCodes.PLUGIN_VERSION_INCOMPATIBLE;
            throw error;
        }
        // Register
     ...
     */
    async initializePlugin(name, config, context) {
        const entry = this.plugins.get(name);
        if (!entry) {
            const error = new Error(`Plugin ${name} not found`);
            error.code = ErrorCodes.PLUGIN_NOT_FOUND;
            throw error;
        }
        ...
        if (plugin.manifest) {
            ...
            if (!configValidation.valid) {
                ...
                entry.error.code = ErrorCodes.PLUGIN_CONFIG_INVALID;
                throw entry.error;
            }
        }
     ...
     */
    async initializeAll(configs, context) {
        const apis = new Map();
