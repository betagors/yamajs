...
import { createCacheRateLimitStore } from "./cache-store.js";
/**
 * Create a rate limiter with the specified store
 */
export function createRateLimiter(store) {
    return {
        async check(request, authContext, config) {
 ...
 */
export async function createRateLimiterFromConfig(config, cacheAdapter) {
    ...
    if (storeType === "cache") {
        ...
        if (cacheAdapter) {
            ...
            console.warn("⚠️  Rate limit store is set to 'cache' but no cache adapter is available. Falling back to memory.");
 ...
 */
function generateRateLimitKey(request, authContext, keyBy) {
...
function extractIpAddress(request) {
    ...
    const original = request._original;
    if (original?.ip) {
        return original.ip;
    }
