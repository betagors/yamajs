import { createRedisOptimizedRateLimitStore } from "./redis-optimized-store.js";
/**
 * Check if a cache adapter is a Redis adapter
 * @internal
 */
function isRedisAdapter(cache) {
    return typeof cache.getRedisClient === "function";
}
/**
...
export class CacheRateLimitStore {
    constructor(cache, keyPrefix = "rate_limit", failClosed = false) {
        this.cache = cache;
        ...
        if (isRedisAdapter(cache)) {
            try {
                ...
                this.optimizedStore = createRedisOptimizedRateLimitStore(redisClient, keyPrefix, failClosed);
            }
            catch {
    ...
    async checkAndIncrement(key, maxRequests, windowMs) {
        ...
        try {
            ...
            if (this.failClosed) {
                ...
                console.error(`Rate limit cache check failed for key ${key} (fail-closed mode): ${error instanceof Error ? error.message : String(error)}`);
                return {
                    allowed: false,
                    remaining: 0,
                    limit: maxRequests,
                ...
                console.warn(`Rate limit cache check failed for key ${key} (fail-open mode): ${error instanceof Error ? error.message : String(error)}`);
                return {
                    allowed: true,
 ...
 */
export function createCacheRateLimitStore(cache, keyPrefix, failClosed) {
