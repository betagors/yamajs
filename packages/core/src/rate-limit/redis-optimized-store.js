 ...
 */
export class RedisOptimizedRateLimitStore {
    constructor(client, keyPrefix = "rate_limit", failClosed = false) {
        this.client = client;
    ...
    async checkAndIncrement(key, maxRequests, windowMs) {
        ...
        try {
            ...
            if (this.failClosed) {
                ...
                console.error(`Rate limit Redis check failed for key ${key} (fail-closed mode): ${error instanceof Error ? error.message : String(error)}`);
                return {
                    allowed: false,
                    remaining: 0,
                    limit: maxRequests,
                ...
                console.warn(`Rate limit Redis check failed for key ${key} (fail-open mode): ${error instanceof Error ? error.message : String(error)}`);
                return {
                    allowed: true,
 ...
 */
export function createRedisOptimizedRateLimitStore(client, keyPrefix, failClosed) {
    return new RedisOptimizedRateLimitStore(client, keyPrefix, failClosed);
}
//# sourceMappingURL=redis-optimized-store.js.map