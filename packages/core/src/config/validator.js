 ...
 */
export function validateConfig(schema, sources) {
    const errors = [];
    const resolved = {};
    ...
    for (const [key, rawDef] of Object.entries(schema)) {
        ...
        let rawValue = sources?.[envVarName] ?? env.getEnv(envVarName);
        ...
        // Validate enum
        if (def.enum && !def.enum.includes(coerced)) {
            errors.push({
                key,
 ...
 */
export function validateConfigOrThrow(schema, context) {
    ...
    if (!result.valid) {
        ...
        const errorList = result.errors
            .map((e) => `  ❌ ${e.message}`)
            .join("\n");
        throw new Error(`${prefix}Configuration validation failed:\n${errorList}\n\n` +
 ...
 */
export function createConfigAccessor(schema) {
