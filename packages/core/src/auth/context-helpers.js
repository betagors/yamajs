 ...
 */
export function matchesPermission(userPerm, required) {
    // Exact match
 ...
 */
export function getRolePermissions(userRoles, rolePermissions) {
 ...
 */
export function createPermissionChecker(userPermissions, rolePermissions, userRoles) {
 ...
 * ```
 */
export function enhanceAuthContext(baseContext, rolePermissions = {}) {
    ...
    return {
         ...
         */
        canAny: (permissions) => permissions.some(perm => can(perm)),
        /**
         * Check if user has all of the specified permissions.
         */
        canAll: (permissions) => permissions.every(perm => can(perm)),
    };
}
/**
 ...
 */
export function extractRolePermissions(authConfig) {
    if (!authConfig?.roles) {
        return {};
    }
    return Object.fromEntries(Object.entries(authConfig.roles).map(([role, config]) => [role, config.permissions]));
}
