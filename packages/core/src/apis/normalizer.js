...
export function normalizeApisConfig(config) {
    const result = {
        rest: [],
    };
    if (!config.apis?.rest) {
    ...
    const hasEndpointsArray = restConfig && typeof restConfig === 'object' && 'endpoints' in restConfig && Array.isArray(restConfig.endpoints);
    if (hasEndpointsArray) {
        // Single REST config
        const singleConfig = restConfig;
        // Skip if disabled
        if (singleConfig.enabled !== false) {
            result.rest.push(normalizeRestConfig('default', singleConfig, config.operations, config.policies, config.schemas));
        }
    }
    else {
        // Multiple named REST configs (object with string keys)
        const namedConfigs = restConfig;
        ...
        for (const [name, restConfigEntry] of Object.entries(namedConfigs)) {
            ...
            if (restConfigEntry) {
                ...
                result.rest.push(normalizeRestConfig(name, restConfigEntry, config.operations, config.policies, config.schemas));
            }
        }
    }
...
function normalizeRestConfig(name, config, operations, policies, allEntities) {
    const endpoints = [];
    ...
    if (operations && config.operations) {
        ...
        if (config.include === "all") {
            ...
            const opNames = new Set(config.operations.map(op => typeof op === "string" ? op : op.operation));
        ...
        for (const endpoint of operationEndpoints) {
            ...
            const operationName = endpoint._operationName;
            if (!operationName) {
