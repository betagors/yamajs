 ...
 */
export function listShadowsWithStatus(configDir: string): ShadowColumnStatus[] {
  const manifest = loadShadowManifest(configDir);
  const now = new Date();
...
export async function cleanupExpiredShadows(
  ...
  const candidates = getCleanupCandidates(configDir, options);
...
export async function restoreShadowColumn(
  ...
  const shadow = getShadowColumn(configDir, table, column);
