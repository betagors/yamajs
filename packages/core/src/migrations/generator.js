 ...
 */
function generateMigrationName(customName) {
 ...
 */
function hasDestructiveOps(steps) {
    return steps.some(step => step.type === "drop_table" ||
 ...
 */
function assessMigrationSafety(steps, diff) {
    const warnings = [];
 ...
 */
function generateRollbackSteps(steps, fromEntities, toEntities) {
    const rollback = [];
 ...
 */
export async function generateMigration(projectDir, currentEntities, options = {}) {
    ...
    if (!destructive) {
        const destructiveSteps = steps.filter(s => s.type === "drop_table" || s.type === "drop_column");
        if (destructiveSteps.length > 0) {
            throw new Error(`Migration contains ${destructiveSteps.length} destructive operation(s). ` +
 ...
 */
export function hasEntityChanges(projectDir, currentEntities) {
    const currentVersion = getCurrentSchemaVersion(projectDir);
    if (!currentVersion) {
