...
export class TypeParser {
    ...
    static parse(typeStr) {
        ...
        if (params) {
            this.parseParameters(baseType, params, fieldType);
        }
        return fieldType;
    }
    /**
     ...
     */
    static parseParameters(type, params, fieldType) {
        switch (type) {
            case 'string':
            case 'text':
            case 'email':
     ...
     */
    static parseStringParams(params, fieldType) {
     ...
     */
    static parseIntParams(params, fieldType) {
     ...
     */
    static parseDecimalParams(params, fieldType) {
     ...
     */
    static parseEnumParams(params, fieldType) {
     ...
     */
    static parseDateTimeParams(params, fieldType) {
        // date(2024-01-01..) or date(..2010-01-01) or date(2020-01-01..2025-01-01)
        const rangeMatch = params.match(/^([^.]+)?\.\.([^.]+)?$/);
        if (rangeMatch) {
            if (rangeMatch[1])
                fieldType.min = rangeMatch[1];
            if (rangeMatch[2])
                fieldType.max = rangeMatch[2];
            return;
        }
        this.parseKeyValueParams(params, fieldType);
    }
     ...
     */
    static parseBinaryParams(params, fieldType) {
        // binary(10mb) or binary(100kb..5mb)
        const rangeMatch = params.match(/^([^.]+)\.\.([^.]+)$/);
        if (rangeMatch) {
            fieldType.min = this.parseSize(rangeMatch[1]);
            fieldType.max = this.parseSize(rangeMatch[2]);
            return;
        }
        const sizeMatch = params.match(/^(\d+)(kb|mb|gb)?$/i);
        if (sizeMatch) {
            fieldType.max = this.parseSize(params);
            return;
        }
        this.parseKeyValueParams(params, fieldType);
    }
     ...
     */
    static parseKeyValueParams(params, fieldType) {
     ...
     */
    static parseGenericParams(params, fieldType) {
        this.parseKeyValueParams(params, fieldType);
    }
    /**
     ...
     * Parse default value
     */
    static parseDefaultValue(value) {
    ...
    static parseExpanded(fieldDef) {
        ...
        if (fieldDef.length !== undefined)
            fieldType.length = fieldDef.length;
        if (fieldDef.precision !== undefined)
            fieldType.precision = fieldDef.precision;
        if (fieldDef.scale !== undefined)
            fieldType.scale = fieldDef.scale;
        if (fieldDef.currency !== undefined)
            fieldType.currency = fieldDef.currency;
        ...
        if (fieldDef.pattern !== undefined)
            fieldType.pattern = fieldDef.pattern;
        if (fieldDef.min !== undefined)
            fieldType.min = fieldDef.min;
        if (fieldDef.max !== undefined)
            fieldType.max = fieldDef.max;
        if (fieldDef.minLength !== undefined)
            fieldType.minLength = fieldDef.minLength;
        if (fieldDef.maxLength !== undefined)
            fieldType.maxLength = fieldDef.maxLength;
        if (fieldDef.minItems !== undefined)
            fieldType.minItems = fieldDef.minItems;
        if (fieldDef.maxItems !== undefined)
            fieldType.maxItems = fieldDef.maxItems;
        if (fieldDef.default !== undefined)
            fieldType.default = fieldDef.default;
        if (fieldDef.defaultFn !== undefined)
            fieldType.defaultFn = fieldDef.defaultFn;
        if (fieldDef.unique === true)
            fieldType.unique = true;
        if (fieldDef.indexed === true)
            fieldType.indexed = true;
        if (fieldDef.readonly === true)
            fieldType.readonly = true;
        if (fieldDef.writeOnly === true)
            fieldType.writeOnly = true;
        if (fieldDef.sensitive === true)
            fieldType.sensitive = true;
        if (fieldDef.generated === true)
            fieldType.generated = true;
        if (fieldDef.autoUpdate === true)
            fieldType.autoUpdate = true;
        if (fieldDef.dbType !== undefined)
            fieldType.dbType = fieldDef.dbType;
        if (fieldDef.description !== undefined)
            fieldType.description = fieldDef.description;
        return fieldType;
    }
}
