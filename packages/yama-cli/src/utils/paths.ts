...
export function getYamaSchemaPath(yamaYamlPath: string): string {
  ...
  while (currentDir !== root && currentDir !== dirname(currentDir)) {
    const nodeModulesPath = join(currentDir, "node_modules", "@yama", "yama-cli", "dist", "yama-cli", "src", "yama.schema.json");
    ...
    const srcPath = join(currentDir, "node_modules", "@yama", "yama-cli", "src", "yama.schema.json");
    if (existsSync(srcPath)) {
      return relative(yamaYamlDir, srcPath).replace(/\\/g, "/");
    }
    
    // Check if we're in a monorepo and the package is in packages/
    const monorepoPath = join(currentDir, "packages", "yama-cli", "src", "yama.schema.json");
  ...
  return "node_modules/@yama/yama-cli/dist/yama-cli/src/yama.schema.json";
}
