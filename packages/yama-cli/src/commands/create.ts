...
import { createSpinner, colors, printBox, success, error, warning, info } from "../utils/cli-utils.js";
...
export async function createCommand(projectName?: string, options: CreateOptions = {}): Promise<void> {
  ...
  if (databaseChoice === "postgresql") {
    selectedPlugins.push("@yama/db-postgres");
    databaseConfig = `
database:
  dialect: postgresql
  url: \${DATABASE_URL}
`;
  ...
  try {
    ...
    if (isInWorkspace && workspaceRootForProject) {
      ...
      devDependencies["yama-cli"] = `file:${relative(projectPath, join(workspaceRootForProject, "packages", "yama-cli")).replace(/\\/g, "/")}`;
    } else {
    ...
    // Use package manager exec for all scripts to ensure yama is found
    // This works better with pnpm workspaces and file: dependencies
    const yamaExec = packageManager === "pnpm" ? "pnpm exec yama" : packageManager === "yarn" ? "yarn yama" : "npx yama";
    ...
    const packageJson: Record<string, unknown> = {
      ...
      scripts: {
        ...
        dev: yamaExec + " dev", // Use exec to ensure yama is found
...
version: 0.1.0${selectedPlugins.length > 0 ? `
plugins:
${selectedPlugins.map(p => `  "${p}": {}`).join("\n")}` : ""}${databaseConfig}
schemas:
  Example:
    fields:
      id:
...
`;
  ...
  printBox(
    ...
    `Plugins: ${selectedPlugins.length > 0 ? colors.cyan(selectedPlugins.join(", ")) : colors.dim("None")}`,
  ...
  if (databaseChoice === "none") {
    ...
    console.log(colors.dim(`   yama plugin install @yama/db-postgres  # Pure JS, recommended`));
  }
  console.log();
}
